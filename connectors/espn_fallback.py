...
import json 
from models.scorecard import BattingEntry,BowlingEntry,Inning,FullScorecardData


BASE_URL = "https://www.espncricinfo.com"
BASE_FIXTURE_AND_RESULT_URL = "match-schedule-fixtures-and-results"
BASE_TEAMS_URL = "teams"
BASE_SQUARDS_URL = "squads"
BASE_SERIES_SQUARD_URL = "series-squads"
BASE_PONIT_TABLE = "points-table-standings"
...
def scrape_team_history(ipl_team_url):
    """
    ...
    TODO:
    - Implement a separate function to scrape detailed team history, including past squads,
      captaincy changes, and notable matches.
    """

    session = requests.Session()
    response = session.get(ipl_team_url, headers=headers)
...
def scrape_ipl_team_links_data(series_url: str) -> list[dict]:
    """Scrapes the list of IPL team links and normalized names from the given IPL series URL."""
    fixture_and_team_url = urljoin(BASE_URL, series_url + '/', BASE_TEAMS_URL)
    session = requests.Session()
    response = session.get(fixture_and_team_url, headers=headers)
        ...
        if "/team/" in href:
            ...
            if span:
                ...
                try:
                    ...
                    team_history = scrape_team_history(full_url)
...
def scrape_squad_players_data(squad_url: str):
    session = requests.Session()
    response = session.get(squad_url, headers=headers)
    ...
    for player_block in soup.find_all("div", class_="ds-flex-1"):
        ...
        if a_tag and role_tag:
            ...
            player_href = urljoin(BASE_URL, a_tag.get("href", "").strip())
...
def scrape_ipl_squards_data(series_url: str) -> list[dict]:
    ...
    seasons_squads_url = urljoin(BASE_URL, series_url + BASE_SQUARDS_URL)

    session = requests.Session()
    response = session.get(seasons_squads_url, headers=headers)
    ...
    for a in soup.find_all("a", href=True):
        ...
        if series_url in href and "/series-squads" in href:
            ...
            if span:
                ...
                _, players = scrape_squad_players_data(full_url)
...
def get_fall_of_wickets_and_did_not_bat(soup):
    part_innings_data = [
...
def scrape_match_full_scorecard_match_flow_data_from_soup(soup):
    match_flow_data = {}
...
def scrape_match_full_scorecard_match_details_data_from_soup(soup):
    match_details_data = {}
...
def scrape_match_full_scorecard_innings_data(full_scorecard_url: str)-> FullScorecardData:
    session = requests.Session()
    response = session.get(full_scorecard_url, headers=headers)
    ...
    for i, inning_div in enumerate(innings_divs):
        ...
        if batting_table:
            ...
            print("This is the length", len(batting_rows))
    ...
    extras = get_extras_data(soup)
    totals = get_total_data(soup)
    match_flow = scrape_match_full_scorecard_match_flow_data_from_soup(soup)
    wickets_dnb = get_fall_of_wickets_and_did_not_bat(soup)
    match_details = scrape_match_full_scorecard_match_details_data_from_soup(soup)
    print("fall of wickets and dnb data ", wickets_dnb)
...
def scrape_full_season_matches_data(match_url: str) -> list[str]:
    ficture_and_base_url = urljoin(match_url + '/', BASE_FIXTURE_AND_RESULT_URL)
    session = requests.Session()
    response = session.get(ficture_and_base_url, headers=headers)
...
def scrape_full_scorecard_links_from_fixture_and_Result(match_url: str) -> list[dict]:
    """
    ...
    fixture_and_result_url = urljoin(match_url + '/', BASE_FIXTURE_AND_RESULT_URL)

    session = requests.Session()
    response = session.get(fixture_and_result_url, headers=headers)
    if response.status_code != 200:
    ...
    for match_div in soup.find_all("div", class_="ds-mb-4"):
        ...
        for a_tag in a_tags:
            ...
            if "/full-scorecard" in href and series_path_with_slash in href:
                full_scorecard_link = urljoin(BASE_URL, href)
                result_span = next(
...
def scraping_full_scorecard_data(series_url: str):
    fixture_and_result = scrape_full_scorecard_links_from_fixture_and_Result(series_url)

    print(json.dumps(fixture_and_result, indent=4), len(fixture_and_result))
    ...
    with open(file_path, "a") as f:
        ...
        for i, match in enumerate(fixture_and_result):
            try:
                ...
                print(link)
                scorecard: FullScorecardData = scrape_match_full_scorecard_innings_data(link)
...
def scraping_full_season_squads_data(series_url: str):
    try:
        squads_data = scrape_ipl_squards_data(series_url)
...
def scraping_full_season_team_data(series_url: str):
    try:
        # Fetch full team data including history and performance
        squads_data = scrape_ipl_team_links_data(series_url)
...
def scrape_ipl_table_data(series_url: str):

    fixture_url = urljoin(BASE_URL, f"{series_url}/{BASE_PONIT_TABLE}")
    session = requests.Session()
    response = session.get(fixture_url, headers=headers)
...
def scraping_complete_point_table_data(series_url: str):
    try:
        # Fetch full team data including history and performance
        table_data = scrape_ipl_table_data(series_url)
...
if __name__ =="__main__":
    for series_url in IPL_SERIES_URLS:
        table_data = scraping_complete_point_table_data(series_url)
    
    print("All Historical data written to respective files successfully ")
